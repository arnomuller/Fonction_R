...
3) Créer un 1er graphique  


::: {.callout-tip title="Astuces :" collapse="false"}
...
- Pour les personnes qui maitrisent ggplot2, vous pouvez modifier les graphiques en les enregistrant dans un objet R, puis en ajoutant les lignes de codes ggplot2 voulues.  
...
Pour l'instant, l'affichage du graphique est trompeur dans Rstudio et ne correspond pas au graphique en sortie.   
Il faut toujours le sauvegarder au format .svg pour pouvoir l'observer en taille réelle.  


:::

4) Sauvegarder le graphique avec `ined_export()` pour avoir une visualisation du graphique dans le format du livre.  

5) Pour voir le graphique final : soit ouvrir avec Inkscape, soit en cliquant-glissant l'image dans un document Word.
...
- **nomfichier :** une chaîne de caractère avec le chemin et le nom du fichier à sauvegarder. (ex : "Sortie/mongraph")  


### Exemple :

Voir ci-dessous avec chaque graphique disponible.
...
::: {.callout-warning title="Sauvegarde :" collapse="false"}

```{r}
#| eval: false
ined_export(baton, "petit", "Sortie/baton_petit")
...
ined_export(baton, "moyen", "Sortie/baton_moyen")
...
```{r}
#| eval: false
ined_export(empile, "petit", "Sortie/empile_petit")
...
ined_export(empile, "moyen", "Sortie/empile_moyen")
...
```{r}
#| eval: false
ined_export(courbe, "petit", "Sortie/courbe_petit")
...
ined_export(courbe, "moyen", "Sortie/courbe_moyen")
